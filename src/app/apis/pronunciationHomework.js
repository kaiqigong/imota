...
router.post('/', async (req, res, next) => {
  try {
    ...
    const homework = new Homework({lessonNo: lessonActivity.lessonNo, courseNo: lessonActivity.courseNo, homeworkName, nickname, time, serverIds, audios, audio, accountId: req.session.loginAccount._id});
    await homework.save();
    res.send(homework);
  } catch (err) {
    next(err);
